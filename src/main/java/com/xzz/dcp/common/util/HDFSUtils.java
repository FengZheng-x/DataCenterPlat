...
public class HDFSUtils {
    ...
    private String getCommand(String action, String prefix) {
        ...
        return String.format("Command: %s%s %s", fsCommand, action, prefix);
    }
    ...
    private String getCommand(String action, String prefix, String suffix) {
        String fsCommand = "hadoop fs -";
        return String.format("Command: %s%s %s %s", fsCommand, action, prefix, suffix);
    }
    ...
    public void mkdir(String dir) {
        try {
            ...
            if (fs.exists(path))
                log.warn("Path {} already exists", dir);
            fs.mkdirs(path);
        } catch (IOException e) {
            log.error(e.getMessage());
        }
